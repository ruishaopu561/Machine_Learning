...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        x_norm = (x - mean_x) / np.sqrt(var_x + eps) #result of bn
        out = gamma * x_norm + beta  #way to train gamma, beta
        # print(x_norm.shape, out.shape, (gamma).shape)
        cache = (x, mean_x, var_x, x_norm, gamma, beta, eps)
        ...
        #######################################################################
        x_norm = (x - running_mean) / np.sqrt(running_var + eps) #result of bn
        out = gamma * x_norm + beta #way to train gamma, beta
...
def batchnorm_backward_alt(dout, cache):
    ...
    N, D = dout.shape
    x, sample_mean, sample_var, x_norm, gamma, beta, eps = cache
    # a = (np.dot(dout, sample_mean.T))
    # b = np.sum(dout, axis=0)
    # print(a.shape, sample_mean.shape, dout.shape, b.shape)
    # dx = gamma / np.sqrt(sample_var + eps) / N * (N * dout - np.dot(a, sample_mean.T) - b)
    dx_norm = dout * gamma
    dsample_var = np.sum(dx_norm * (-0.5 * x_norm / (sample_var + eps)), axis=0)
    dsample_mean = np.sum(-dx_norm / np.sqrt((sample_var + eps)), axis=0) + \
                  dsample_var * np.sum(-2.0/N * (x - sample_mean), axis=0)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    h_steps = (H - HH) // stride + 1
    w_steps = (W - WW) // stride + 1

    out = []
    for i in range(N):
        x_i = x_pad[i]
        middle_out = np.zeros(((F, h_steps, w_steps)))
        for j in range(F):
              w_j = w[j].reshape(-1,)
              for height in range(h_steps):
                  for width in range(w_steps):
                      x_f = x_i[:, height * stride : height * stride + HH, width * stride : width * stride + WW]
                      middle_out[j, height, width] = w_j.dot(x_f.reshape(-1,)) + b[j]
        out.append(middle_out)
    out = np.array(out)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def conv_backward_naive(dout, cache):
    ...
    for j in range(out_h):
        for k in range(out_w):
            h_steps = min(j * stride, H + 2 * pad - HH)
            w_steps = min(k * stride, W + 2 * pad - WW)
            for i in range(N):
                dx_pad[i, :, h_steps: h_steps + HH, w_steps: w_steps + WW] += np.sum((dout[i, :, j, k])[:, None, None, None] * w, axis=0)
            for i in range(F):
                dw[i, :, :, :] += np.sum(x_pad[:, :, h_steps: h_steps + HH, w_steps: w_steps + WW] * (dout[:, i, j, k])[:, None, None, None], axis=0)
        ...
        for c in range(C):  
            for i in range(h_step):
                for j in range(w_step):
                    out[n, c, i, j] = x[n, c, i * stride : i * stride + stride, j * stride : j * stride + stride].max()
    out = np.array(out)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for c in range(C):  
            for i in range(h_step):
                for j in  range(w_step):
                    a = np.argmax(x[n, c, i * stride : i * stride + stride, j * stride : j * stride + stride].reshape(-1,), axis=0)
                    dx[n, c, a // stride + i * stride, a % stride + j * stride] = dout[n, c, i, j]                
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    ###########################################################################
    N, C, H, W = x.shape
    x_group = x.reshape(N, G, C // G, H, W)  #分为C//G组
    ...
    x_norm = x_groupnorm.reshape(N, C, H, W)  #还原维度
